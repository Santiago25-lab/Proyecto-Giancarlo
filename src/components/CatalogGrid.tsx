...
export default function CatalogGrid() {
  ...
  return (
    <section ref={containerRef} className="py-16 px-4 md:px-8 max-w-7xl mx-auto bg-[#FBFBFD] min-h-screen">
      {/* Upper Brand Section */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-16">
        <div>
          ...
          <h2 className="brand-title text-4xl md:text-6xl font-black tracking-tighter text-black mt-2">
            EXCLUSIVIDAD <br className="hidden md:inline" /> EN TU <span className="text-red-600">MANO</span>
          </h2>
        </div>
        <p className="brand-desc text-gray-500 max-w-md text-sm md:text-base font-medium">
      ...
      <div className="search-bar-wrap max-w-lg mb-10 relative z-30">
        <div className="relative flex items-center bg-white border border-gray-200 focus-within:border-red-600 focus-within:ring-2 focus-within:ring-red-600/10 rounded-2xl shadow-sm px-4 py-3.5 transition-all duration-300 backdrop-blur-md bg-white/70">
          ...
          />
          {searchQuery && (
            <button onClick={() => setSearchQuery('')} className="p-1 hover:bg-gray-100 rounded-full text-gray-400 transition">
              <X className="w-4 h-4" />
            </button>
          )}
        </div>
        ...
        {showSuggestions && searchQuery.trim() !== '' && (
          <div className="absolute top-full left-0 right-0 mt-2 bg-white/95 backdrop-blur-xl border border-gray-150 rounded-2xl shadow-2xl z-40 max-h-72 overflow-y-auto p-2 space-y-1 select-none animate-in fade-in slide-in-from-top-2 duration-200">
            {suggestions.length > 0 ? (
              suggestions.slice(0, 5).map((prod) => (
                <div
                  ...
                  className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-red-50/50 cursor-pointer transition duration-200"
                >
                  <img src={prod.image_url} alt={prod.name} className="w-10 h-10 object-contain bg-[#F5F5F7] rounded-lg p-0.5" />
                  <div className="flex-grow text-left">
                    <span className="block text-xs font-extrabold text-black line-clamp-1">{prod.name}</span>
                    <span className="text-[10px] text-red-600 font-bold">${prod.price.toLocaleString('es-CO')} COP</span>
                  </div>
                  <span className="text-[9px] bg-black text-white font-bold uppercase tracking-wider px-2 py-0.5 rounded-full shrink-0">
                    {prod.category}
                  </span>
                </div>
              ))
      ...
      <div className="flex flex-wrap gap-2.5 mb-12 overflow-x-auto pb-4 scrollbar-none">
        {CATEGORIES.map((cat) => (
          <button
            ...
            className={`category-pill flex items-center gap-2 px-5 py-3 rounded-full text-sm font-semibold transition-all duration-300 ${
      ...
      {loading ? (
        <div className="py-24 text-center text-gray-500 flex flex-col items-center justify-center gap-4">
          ...
          <span className="font-semibold text-lg">Cargando dispositivos exclusivos...</span>
        </div>
      ) : filteredProducts.length === 0 ? (
        <div className="py-20 text-center bg-white border border-gray-150 rounded-3xl p-8 max-w-lg mx-auto shadow-sm">
          <ShoppingBag className="w-12 h-12 text-gray-300 mx-auto mb-4" />
          <h3 className="text-xl font-bold text-black mb-2">No hay productos en esta categoría</h3>
          ...
          <AnimatePresence mode="popLayout">
            {filteredProducts.map((product) => (
              <motion.div
                ...
                className="product-card bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-xl hover:border-gray-200 transition-all duration-500 flex flex-col group cursor-pointer"
              >
                {/* Image Area with Red Glow on hover */}
                <div className="aspect-[4/3] bg-[#F5F5F7] relative overflow-hidden flex items-center justify-center p-6 border-b border-gray-50">
                  {product.image_url ? (
                    <img
                  ...
                  <div className="absolute top-4 left-4 flex flex-col gap-2">
                    <span className="bg-black/90 text-white text-[10px] font-bold uppercase tracking-wider px-3 py-1 rounded-full shadow-sm">
                      {product.category}
                    </span>
                ...
                <div className="p-6 md:p-8 flex-grow flex flex-col justify-between space-y-6">
                  <div>
                    <h3 className="text-xl md:text-2xl font-extrabold text-black group-hover:text-red-600 transition-colors duration-300 line-clamp-1">
                      {product.name}
                    </h3>
                  ...
                  <div className="pt-4 border-t border-gray-50 flex items-center justify-between">
                    <div>
                      <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider block">Precio Especial</span>
                      <span className="text-xl md:text-2xl font-black text-black">
                        ${product.price.toLocaleString('es-CO')} <span className="text-xs font-semibold text-gray-500">COP</span>
                      </span>
                    </div>

                    <button
                      className="inline-flex items-center gap-2 font-bold text-sm px-5 py-3.5 rounded-2xl bg-gray-50 border border-gray-200 text-black group-hover:bg-red-600 group-hover:text-white group-hover:border-red-600 transition-all duration-300 shadow-sm"
                    >
        ...
        {selectedProduct && (
          ...
          >
            <motion.div
              ...
              className="bg-white w-full max-w-5xl rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row relative max-h-[90vh] md:h-[580px]"
            ...
            >
              ...
              <button
                onClick={closeModal}
                className="absolute top-4 right-4 z-20 p-2 bg-gray-150/80 hover:bg-gray-200 text-gray-700 rounded-full transition duration-300 shadow-sm"
              >
                <X className="w-5 h-5" />
              </button>

              {/* LEFT SIDE: CAROUSEL GALLERY */}
              <div className="w-full md:w-[55%] bg-[#F5F5F7] p-6 flex flex-col justify-between relative min-h-[350px] md:min-h-0 md:h-full">
                {/* Arrow Navigation */}
                {selectedProduct.images.length > 1 && (
                  <>
                    <button
                      onClick={() => prevImage(selectedProduct.images.length)}
                      className="absolute left-4 top-1/2 -translate-y-1/2 z-10 p-2.5 bg-white/90 hover:bg-white text-black rounded-full shadow-md transition"
                    >
                      <ChevronLeft className="w-5 h-5 font-bold" />
                    </button>
                    <button
                      onClick={() => nextImage(selectedProduct.images.length)}
                      className="absolute right-4 top-1/2 -translate-y-1/2 z-10 p-2.5 bg-white/90 hover:bg-white text-black rounded-full shadow-md transition"
                    >
                  ...
                  <div className="flex justify-center gap-2 mt-4 overflow-x-auto py-2 scrollbar-none shrink-0">
                    {selectedProduct.images.map((image, idx) => (
                      <button
                        ...
                        className={`w-14 h-14 rounded-xl border-2 bg-white flex items-center justify-center overflow-hidden transition-all shrink-0 ${
                          activeImageIndex === idx ? 'border-red-600 scale-105 shadow-sm' : 'border-gray-200 hover:border-gray-300'
                        }`}
                      >
              ...
              <div className="w-full md:w-[45%] flex flex-col bg-white h-[350px] md:h-full justify-between">
                
                {/* Scrollable Content Container */}
                <div className="flex-grow overflow-y-auto p-6 md:p-8 space-y-6 select-none scrollbar-thin scrollbar-thumb-gray-200 hover:scrollbar-thumb-gray-300">
                  
                  {/* Category & Availability Badges */}
                  <div className="flex items-center gap-2">
                    <span className="bg-red-50 text-red-600 border border-red-100 text-[10px] font-bold uppercase tracking-wider px-3 py-1 rounded-full">
                      {selectedProduct.category}
                    </span>
                    <span className="bg-gray-100 text-gray-600 border border-gray-200 text-[10px] font-bold uppercase tracking-wider px-3 py-1 rounded-full">
                      Original Apple
                    </span>
                  </div>
                  ...
                  {/* Pricing block */}
                  <div>
                    <span className="text-[10px] text-gray-400 uppercase tracking-wider font-bold block mb-1">Valor de Inversión</span>
                    <span className="text-2xl md:text-3xl font-black text-red-600">
                      ${selectedProduct.price.toLocaleString('es-CO')} <span className="text-xs font-semibold text-gray-500">COP</span>
                    </span>
                  </div>

                  {/* Specifications List */}
                  <div className="space-y-3.5 border-t border-b border-gray-100 py-5 my-2">
                    <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Detalles Premium</h4>
                  ...
                  <div className="space-y-1.5 pb-4">
                    <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Descripción del Producto</h4>
                ...
                <div className="p-6 border-t border-gray-100 flex items-center justify-between gap-4 bg-white shrink-0 shadow-[0_-4px_20px_-10px_rgba(0,0,0,0.05)]">
                  <div className="text-[10px] text-gray-400 font-bold uppercase tracking-wider flex items-center gap-1.5 shrink-0">
